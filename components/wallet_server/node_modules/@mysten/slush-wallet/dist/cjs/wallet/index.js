...
var _id, _events, _accounts, _origin, _walletName, _icon, _name, _signTransactionBlock, _signTransaction, _signAndExecuteTransaction, _signPersonalMessage, _on, _SlushWallet_instances, setAccounts_fn, _connect, _disconnect, getNewPopupChannel_fn;
...
async function getAccountsFromSession(session) {
  const { payload } = await (0, import_window_wallet_core.decodeJwtSession)(session);
    ...
    __privateAdd(this, _connect, async (input) => {
      if (input?.silent) {
        try {
          ...
          if (accounts.length) {
            __privateMethod(this, _SlushWallet_instances, setAccounts_fn).call(this, accounts);
          }
        } catch (error) {
        }
      ...
      __privateMethod(this, _SlushWallet_instances, setAccounts_fn).call(this, await getAccountsFromSession(response.session));
      return { accounts: this.accounts };
    });
...
async function registerSlushWallet(name, {
  origin,
